                          ...
                          const std::array<double, 3>& L, const double& rc2) 
{
  ...
  while (a < N) {
    ...
    // Scale random coordinates to 1/2 box size
    xp[0] *= 0.25 * L[0];
    xp[1] *= 0.25 * L[1];
    xp[2] *= 0.25 * L[2];
    ...
    bool flag = true;
    for (int b = 0; b < N; b++) {
        double dr2 = (std::pow(xp[0]-R[0][b],2) + \
...
/* --------------------------------------------------------------------------------------------- */
void calc_routines::initialise_velocities(std::vector<std::vector<double>>& V, const int& N,\
...
/* --------------------------------------------------------------------------------------------- */
void calc_routines::pbc(std::vector<std::vector<double>>& R, std::vector<double>&iR,\
...
/* --------------------------------------------------------------------------------------------- */
std::vector<std::vector<double>> calc_routines::force_routine(
